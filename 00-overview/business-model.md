...
```mermaid
flowchart LR
  Partner["Operating partnership"] --> Improve["Improve the business"]
  Improve --> Optionality["Create transition optionality"]
  Optionality --> Buy["Seller-financed or staged buyout"]
  Optionality --> Employ["Owner stays part time"]
  Optionality --> Continue["Continue support model"]
  Buy --> Platform["Platform business"]
  Employ --> Platform
  Continue --> Platform
  Platform --> AddOns["Add complementary trades"]
  AddOns --> Group["Larger construction operating group"]
```
